...
MagicCommand::MagicCommand(EntryPoint entry_point)
    : output_(), entry_point_(entry_point), command_undefined_(true) {
  SET_TIMER(__PRETTY_FUNCTION__);
...
// Method      :  Run
//
// Notes       :  1. This is a collective operation and an entry point to call libyt
// defined magic command (%libyt).
//                2. stringstream is slow and string copying is slow, but ..., too lazy to
//                do that.
...
//
// Arguments   :  const std::string& command : command to run (default is "", which will
// get command from root).
//
// Return      : MagicCommandOutput& : output data are generated and cached while calling
// methods.
//-------------------------------------------------------------------------------------------------------
MagicCommandOutput& MagicCommand::Run(const std::string& command) {
  ...
  if (command_undefined_) {
    ...
    output_.error = std::string("Unknown libyt command : ") + command_ +
                    std::string("\n") + std::string("(Type %libyt help for help ...)");
  }
  if (write_to_history && mpi_rank_ == mpi_root_) {
    LibytProcessControl::Get().python_shell_.UpdateHistory(std::string("# ") + command_ +
                                                           std::string("\n"));
  }
...
// Method     :  Exit
//
// Notes      :  1. Generate helper msg for JupyterKernel if this method is called, since
// this is not
//                  supported in JupyterKernel.
//               2. Exit libyt prompt or reloading script mode in interactive mode and
//               reloading script.
...
int MagicCommand::Exit() {
  ...
  if (entry_point_ == kLibytJupyterKernel) {
    output_.exit_entry_point = false;
    output_.status = "Error";
    output_.error = std::string("\"%libyt exit\" is not supported in Jupyter, please use "
                                "frontend UI 'shutdown' to exit libyt kernel.");

    return YT_FAIL;
  } else if (entry_point_ == kLibytReloadScript) {
    output_.exit_entry_point = false;
    output_.status = "Error";
    output_.error = std::string("\"%libyt exit\" is not supported in reloading script, "
                                "please create flagged file to exit.");
...
int MagicCommand::GetStatusHtml() {
  ...
  const char* kSuccessCell =
      "<td><span "
      "style=\"color:#28B463;font-weight:bold;font-family:'arial'\">Success</span></td>";
  const char* kFailedCell =
      "<td><span "
      "style=\"color:#E74C3C;font-weight:bold;font-family:'arial'\">Failed</span></td>";
  const char* kIdleCell =
      "<td><span "
      "style=\"color:#2874A6;font-weight:bold;font-family:'arial'\">Idle</span></td>";
  const char* kUnknownCell =
      "<td><span "
      "style=\"color:#A569BD;font-weight:bold;font-family:'arial'\">Unknown</span></td>";
  const char* kWillRunCell =
      "<td><span "
      "style=\"color:#F1C40F;font-weight:bold;font-family:'arial'\">V</span></td>";
  const char* kWillIdleCell =
      "<td><span "
      "style=\"color:#F1C40F;font-weight:bold;font-family:'arial'\">X</span></td>";

  output_.output += "<table style=\"width: 100%\"><tr><th>Inline "
                    "Function</th><th>Status</th><th>Run</th></tr>";

  for (int i = 0; i < LibytProcessControl::Get().function_info_list_.GetSize(); i++) {
    // Get function name
    output_.output +=
        "<tr><td style=\"text-alight: left;\"><span style=\"font-family:'Courier New'\">";
    output_.output += LibytProcessControl::Get().function_info_list_[i].GetFunctionName();
    output_.output += "</span></td>";

    // Get function status
    FunctionInfo::ExecuteStatus status =
    ...
    FunctionInfo::RunStatus run =
        LibytProcessControl::Get().function_info_list_[i].GetRun();
...
int MagicCommand::GetStatusText() {
  ...
  for (int i = 0; i < LibytProcessControl::Get().function_info_list_.GetSize(); i++) {
    ...
    if (status == FunctionInfo::ExecuteStatus::kFailed) {
      ...
      output_.error += std::string("Unknown status code ") + std::to_string(status) +
                       std::string("\n");
    }
...
int MagicCommand::GetHelpMsgMarkdown() {
  ...
  output_.output = std::string(
      ...
      "| load | filename | Load and run Python file `filename` in imported script's "
      "namespace. |\n"
      "| export | filename | Export history to `filename`. |\n"
      "| status |  | Get overall function status. <br> - \"Status\" indicates the "
      "execute status call by "
      "libyt API. <br> - \"Run\" indicates the the function will run automatically. |\n"
      "| status | function_name | Get `function_name` status. |\n"
      "| run | function_name [arg1, arg2, ...] | Run `function_name` automatically by "
      "calling "
...
int MagicCommand::GetHelpMsgText() {
  ...
  if (entry_point_ == EntryPoint::kLibytInteractiveMode) {
    output_.output = "Usage: \033[33;1;4m%libyt COMMAND\033[0;37m\n"
                     "Commands:\n"
                     "  \033[32;1mhelp  \033[1;31m              \033[0;37m              "
                     "Print help messages.\n"
                     "  \033[32;1mexit  \033[1;31m              \033[0;37m              "
                     "Exit entry point.\n"
                     "  \033[32;1mload  \033[1;31m  <file name> \033[0;37m              "
                     "Load and run Python file <file name>.\n"
                     "  \033[32;1mexport\033[1;31m  <file name> \033[0;37m              "
                     "Export history to <file name>.\n"
                     "  \033[32;1mstatus\033[1;31m              \033[0;37m              "
                     "Get all function status.\n"
                     "  \033[32;1mstatus\033[1;31m  <function>  \033[0;37m              "
                     "Get <function> status.\n"
                     "  \033[32;1mrun   \033[1;31m  <function>  \033[0;37m "
                     "[\033[1;31marg1\033[0;37m, ...]  Run "
                     "<function>(args) automatically in next iteration.\n"
                     "  \033[32;1midle  \033[1;31m  <function>  \033[0;37m              "
                     "Make <function> idle in next "
                     "iteration.\n";
  } else {
    output_.output =
        "Usage:  %libyt COMMAND\n"
        ...
        "  run     <function>  [arg1, ...]  Run <function>(args) automatically in next "
        "iteration.\n"
...
//               2. Run the Python file line-by-line, if variable names already exist in
//               inline script's
//                  namespace, it will be overwritten by this file.
//               3. Parse functions in script and add to
//               LibytProcessControl::Get().function_info_list_. If function name already
//                  exists in the list, the source code in
//                  libyt.interactive_mode["func_body"] will be updated and overwritten.
//               4. Character in the file loaded cannot exceed INT_MAX.
//               5. CAUTION: Currently, only root rank gets the full error msg and output,
//               so broadcast
//                  the results. (TODO: probably should update the data structure in
//                  execute_file.)
//
// Arguments  :  const std::vector<std::string>& args : Full magic commands. (ex: %libyt
// load filename.py)
//
// Return     : YT_SUCCESS or YT_FAIL
//-------------------------------------------------------------------------------------------------------
int MagicCommand::LoadScript(const std::vector<std::string>& args) {
  ...
  if (mpi_rank_ == mpi_root_) {
    ...
    if (!stream) {
      ...
      output_.error +=
          std::string("Loading script '") + args[2] + std::string("' ... failed.\n");
      return YT_FAIL;
    }
    std::string line;
    std::stringstream ss;
    ...
    CodeValidity code_validity =
        LibytPythonShell::CheckCodeValidity(ss.str(), false, args[2].c_str());
...
#ifndef SERIAL_MODE
          if (entry_point_ == kLibytInteractiveMode) {
            output_.output += std::string("\033[1;34m[MPI Process ") + std::to_string(r) +
                              std::string("]\n\033[0;37m");
          } else if (entry_point_ == kLibytJupyterKernel) {
            output_.output += std::string("\033[1;34m[MPI Process ") + std::to_string(r) +
                              std::string("]\n\033[0;30m");
            ...
            output_.output +=
                std::string("[MPI Process ") + std::to_string(r) + std::string("]\n");
          }
#endif
          output_.output +=
              (output[r].output.empty() ? "(None)\n" : output[r].output) + "\n";
        }
      }
      ...
      if (!all_error_is_none) {
        for (int r = 0; r < mpi_size_; r++) {
          ...
          if (entry_point_ == kLibytInteractiveMode) {
            ...
            output_.error += std::string("[MPI Process ") + std::to_string(r) +
                             std::string(" -- Error Msg]\n");
          }
#endif
          output_.error +=
              (output[r].error.empty() ? "(None)\n" : output[r].error) + "\n";
        }
      }
    } else {
#ifndef SERIAL_MODE
  ...
  // Get function list defined inside the script, add the function name to list if it
  // doesn't exist and set to idle
  std::vector<std::string> func_list =
      LibytPythonShell::get_funcname_defined(args[2].c_str());
  for (int i = 0; i < (int)func_list.size(); i++) {
    LibytProcessControl::Get().function_info_list_.AddNewFunction(
        func_list[i], FunctionInfo::RunStatus::kWillIdle);
  }
  ...
  if (python_run_successfully) {
    output_.status = "Success";
    output_.output +=
        std::string("Loading script '") + args[2] + std::string("' ... done\n");
  } else {
    output_.status = "Error";
    output_.output +=
        std::string("Loading script '") + args[2] + std::string("' ... failed\n");
...
int MagicCommand::ExportScript(const std::vector<std::string>& args) {
  ...
  output_.status = "Success";
  output_.output =
      std::string("Exporting history to '") + args[2] + std::string("' ... done\n");
...
int MagicCommand::SetFunctionRun(const std::vector<std::string>& args) {
  ...
  if (args.size() < 3) {
    output_.status = "Error";
    output_.error = std::string(
        "Usage: %libyt run function_name [arg1, arg2, ...]\n"
  ...
  if (args.size() != 3) {
    ...
    for (size_t i = 3; i < args.size(); i++) {
      if (!wrapper_detected) {
        ...
        const char* wrapper =
            LibytProcessControl::Get().function_info_list_[index].GetWrapper();
    ...
    if (unable_to_wrapped) {
      ...
      output_.error +=
          std::string("Please avoid using both \"\"\" and ''' for triple quotes\n");
      return YT_FAIL;
    }
  }
  LibytProcessControl::Get().function_info_list_[index].SetRun(
      FunctionInfo::RunStatus::kWillRun);
  LibytProcessControl::Get().function_info_list_[index].SetInputArgs(input_args);

  output_.status = "Success";
  output_.output +=
      std::string("Function '") + args[2] + std::string("' set to run ... done\n");
  output_.output += std::string("Run ") +
                    LibytProcessControl::Get()
                        .function_info_list_[index]
                        .GetFunctionNameWithInputArgs() +
...
int MagicCommand::SetFunctionIdle(const std::vector<std::string>& args) {
  ...
  if (args.size() != 3) {
    output_.status = "Error";
    output_.error =
        std::string("Usage: %libyt idle function_name\n"
  ...
  output_.status = "Success";
  output_.output +=
      std::string("Function '") + args[2] + std::string("' set to idle ... done\n");
...
int MagicCommand::GetFunctionStatusMarkdown(const std::vector<std::string>& args) {
  ...
  if (args.size() != 3) {
    output_.status = "Error";
    output_.error =
        std::string("Usage: %libyt status function_name\n"
  ...
  FunctionInfo::ExecuteStatus status =
      LibytProcessControl::Get().function_info_list_[index].GetAllStatus();
  if (mpi_rank_ == mpi_root_) {
    ...
    if (LibytProcessControl::Get().function_info_list_[index].GetRun() ==
        FunctionInfo::RunStatus::kWillRun) {
      output_.output += std::string("`") +
                        LibytProcessControl::Get()
                            .function_info_list_[index]
    ...
    std::string func_body =
        LibytProcessControl::Get().function_info_list_[index].GetFunctionBody();
    std::size_t start_pos = 0, found;
    while (true) {
      found = func_body.find('\n', start_pos);
      if (found != std::string::npos) {
        output_.output += std::string("  ") +
                          func_body.substr(start_pos, found - start_pos) +
                          std::string("\n");
      } else {
        output_.output +=
            std::string("  ") + func_body.substr(start_pos) + std::string("\n");
        break;
      }
      start_pos = found + 1;
    }
  ...
  if (status == FunctionInfo::ExecuteStatus::kFailed) {
    std::vector<std::string> output_error =
        LibytProcessControl::Get().function_info_list_[index].GetAllErrorMsg();
    if (mpi_rank_ == mpi_root_) {
      ...
      for (size_t r = 0; r < output_error.size(); r++) {
        ...
        output_.output += std::string("<details><summary>MPI Process ") +
                          std::to_string(r) + (" </summary><p>");

        std::size_t start_pos = 0, found;
        while (true) {
          found = output_error[r].find('\n', start_pos);
          if (found != std::string::npos) {
            output_.output += output_error[r].substr(start_pos, found - start_pos) +
                              std::string("<br>");
          } else {
...
//               2. libyt.interactive_mode["func_err_msg"] only stores function's error
//               msg when using
...
int MagicCommand::GetFunctionStatusText(const std::vector<std::string>& args) {
  ...
  if (mpi_rank_ == mpi_root_) {
    // Get status
    output_.output +=
        LibytProcessControl::Get().function_info_list_[index].GetFunctionName() +
        std::string(" ... ");
    if (status == FunctionInfo::ExecuteStatus::kSuccess) {
  ...
  // Get error msg if it failed when running in yt_run_Function/yt_run_FunctionArguments.
  // (collective call)
  if (status == FunctionInfo::ExecuteStatus::kFailed) {
    std::vector<std::string> output_error =
        LibytProcessControl::Get().function_info_list_[index].GetAllErrorMsg();
      ...
      for (size_t r = 0; r < output_error.size(); r++) {
#ifndef SERIAL_MODE
        if (entry_point_ == kLibytInteractiveMode) {
          output_.output += std::string("\033[1;36m  [ MPI process ") +
                            std::to_string(r) + (" ]\033[0;37m\n");
        } else {
          output_.output +=
              std::string("  [ MPI process ") + std::to_string(r) + (" ]\n");
        }
#endif
        ...
        while (true) {
          ...
          if (found != std::string::npos) {
            output_.output += std::string("    ");
            output_.output +=
                output_error[r].substr(start_pos, found - start_pos) + std::string("\n");
          } else {
            output_.output += output_error[r].substr(start_pos);
            break;
          }

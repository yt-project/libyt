...
void DataStructureAmr::InitializeMpiHierarchyDataType() {
  ...
  MPI_Datatype types[7] = {
      MPI_DOUBLE, MPI_DOUBLE, MPI_LONG, MPI_LONG, MPI_INT, MPI_INT, MPI_INT};
  MPI_Type_create_struct(
      7, lengths, displacements, types, &DataStructureAmr::mpi_hierarchy_data_type_);
  MPI_Type_commit(&DataStructureAmr::mpi_hierarchy_data_type_);
#endif
}

void DataStructureAmr::SetMpiInfo(const int mpi_size, const int mpi_root,
                                  const int mpi_rank) {
  mpi_size_ = mpi_size;
  mpi_root_ = mpi_root;
  mpi_rank_ = mpi_rank;
...
DataStructureOutput DataStructureAmr::AllocateFieldList(int num_fields) {
  if (num_fields < 0) {
    return {DataStructureStatus::kDataStructureFailed,
            "Number of fields should not be negative."};
  }
...
//                2. Since we need particle type and its number of attributes, we need to
//                pass in the
//                   particle type list. And num_par_types_ tracks the array length of
//                   particle_list_.
//                3. Make sure particle_list_ is properly freed before new allocation.
//-------------------------------------------------------------------------------------------------------
DataStructureOutput DataStructureAmr::AllocateParticleList(int num_par_types,
                                                           yt_par_type* par_type_list) {
  if (num_par_types < 0) {
    return {DataStructureStatus::kDataStructureFailed,
            "Number of particle types should not be negative."};
  }
...
//                4.
//                num_grids_locals_/num_grids_local_field_data_/num_grids_local_par_data_
...
DataStructureOutput DataStructureAmr::AllocateGridsLocal(int num_grids_local,
  ...
  if (num_grids_local < 0) {
    return {DataStructureStatus::kDataStructureFailed,
            "Number of local grids should not be negative."};
  }
  if (num_fields < 0) {
    return {DataStructureStatus::kDataStructureFailed,
            "Number of fields should not be negative."};
...
DataStructureOutput DataStructureAmr::AllocateFullHierarchyStorageForPython(
  ...
  if (num_grids < 0) {
    return {DataStructureStatus::kDataStructureFailed,
            "Number of grids should not be negative."};
  }
  if (num_par_types < 0) {
    return {DataStructureStatus::kDataStructureFailed,
            "Number of particle types should not be negative."};
  ...
  np_dim[1] = 3;
  PyObject* py_grid_left_edge = numpy_controller::ArrayToNumPyArray(
      2, np_dim, YT_DOUBLE, grid_left_edge_, false, false);
  PyObject* py_grid_right_edge = numpy_controller::ArrayToNumPyArray(
      2, np_dim, YT_DOUBLE, grid_right_edge_, false, false);
  PyObject* py_grid_dimensions = numpy_controller::ArrayToNumPyArray(
      2, np_dim, YT_INT, grid_dimensions_, false, false);

  np_dim[1] = 1;
  PyObject* py_grid_parent_id = numpy_controller::ArrayToNumPyArray(
      2, np_dim, YT_LONG, grid_parent_id_, false, false);
  PyObject* py_grid_levels =
      numpy_controller::ArrayToNumPyArray(2, np_dim, YT_INT, grid_levels_, false, false);
  PyObject* py_proc_num =
      numpy_controller::ArrayToNumPyArray(2, np_dim, YT_INT, proc_num_, false, false);
  PyObject* py_par_count_list;
  if (num_par_types > 0) {
    np_dim[1] = num_par_types;
    py_par_count_list = numpy_controller::ArrayToNumPyArray(
        2, np_dim, YT_LONG, par_count_list_, false, false);
  }

  // Bind them to libyt.hierarchy
  // Even though the pointer is de-referenced, still need to freed it in the memory
  // ourselves at freed
#ifndef USE_PYBIND11
...
DataStructureOutput DataStructureAmr::GatherAllHierarchy(
  ...
  if (num_grids != num_grids_) {
    ...
    std::string error = "Sum of number of local grids in all ranks is not equal to the "
  ...
  BigMpiAllgatherv<yt_hierarchy>(all_num_grids_local,
                                 (void*)hierarchy_local,
                                 DataStructureAmr::mpi_hierarchy_data_type_,
                                 (void*)hierarchy_full);
  for (int s = 0; s < num_par_types_; s++) {
    BigMpiAllgatherv<long>(all_num_grids_local,
                           (void*)particle_count_list_local[s],
                           MPI_LONG,
...
//                2. TODO: Due to bad api design, field_list_ is created under
//                libyt.param_yt["field_list"]
//                         as a dictionary.
//                3. If using Python C API, PyUnicode_FromString is Python-API >= 3.5, and
...
//                   (3) Structure of each element in libyt.param_yt["field_list"]
//                   dictionary:
//            field_list_dict    field_info_dict        info_list     name_alias_list
//                   |               |                      |               |
//                   { <field_name>: {"attribute"       : ( <field_unit>,
//                   (<field_name_alias>, ), <field_display_name>)
//                                    "field_type"      :  <field_type>,
//                                    "contiguous_in_x" :  true / false
//                                    "ghost_cell"      : ( beginning of 0-dim, ending of
//                                    0-dim,
//                                                          beginning of 1-dim, ending of
//                                                          1-dim, beginning of 2-dim,
//                                                          ending of 2-dim  ) },
//                   }
//-------------------------------------------------------------------------------------------------------
DataStructureOutput DataStructureAmr::BindFieldListToPython(
    PyObject* py_dict, const std::string& py_dict_name) const {
  if (check_data_) {
  ...
  for (int i = 0; i < num_fields_; i++) {
    ...
    if (PyList_Append(info_list, val) != 0) {
      ...
      Py_DECREF(info_list);
      Py_XDECREF(val);
      std::string error = "(field, field_unit) = (" +
                          std::string((field_list_)[i].field_name) + ", " +
                          std::string((field_list_)[i].field_unit) +
                          "), failed to append field_unit to list!\n";
      return {DataStructureStatus::kDataStructureFailed, error};
    }
    Py_DECREF(val);
    ...
    for (int j = 0; j < (field_list_)[i].num_field_name_alias; j++) {
      ...
      if (PyList_Append(name_alias_list, val) != 0) {
        ...
        Py_XDECREF(val);
        std::string error = "(field, field_name_alias) = (" +
                            std::string((field_list_)[i].field_name) + ", " +
    ...
    if ((field_list_)[i].field_display_name == nullptr) {
      if (PyList_Append(info_list, Py_None) != 0) {
        ...
        Py_DECREF(field_info_dict);
        Py_DECREF(info_list);
        std::string error = "(field, field_display_name) = (" +
                            std::string((field_list_)[i].field_name) +
    ...
    if (PyDict_SetItemString(field_info_dict, "attribute", info_list) != 0) {
      ...
      Py_DECREF(info_list);
      std::string error = "(field) = (" + std::string((field_list_)[i].field_name) +
                          "), failed to add key-value pair 'attribute' and info list "
    ...
    if (PyDict_SetItemString(field_info_dict, "field_type", val) != 0) {
      ...
      Py_DECREF(field_info_dict);
      Py_XDECREF(val);
      std::string error = "(field, field_type) = (" +
                          std::string((field_list_)[i].field_name) + ", " +
    ...
    if ((field_list_)[i].contiguous_in_x) {
      if (PyDict_SetItemString(field_info_dict, "contiguous_in_x", Py_True) != 0) {
        Py_DECREF(field_list_dict);
        Py_DECREF(field_info_dict);
        std::string contiguous_in_x_str =
            ((field_list_)[i].contiguous_in_x) ? "true" : "false";
        std::string error = "(field, contiguous_in_x) = (" +
                            std::string((field_list_)[i].field_name) + ", " +
                            contiguous_in_x_str +
                            "), failed to add key-value pair 'contiguous_in_x'!\n";
        return {DataStructureStatus::kDataStructureFailed, error};
      }
    } else {
  ...
  if (PyDict_SetItemString(py_dict, "field_list", field_list_dict) != 0) {
    Py_DECREF(field_list_dict);
    std::string error =
        "Inserting dictionary 'field_list' to '" + py_dict_name + "' failed!\n";
    return {DataStructureStatus::kDataStructureFailed, error};
  }
  ...
  for (int i = 0; i < num_fields_; i++) {
    ...
    py_field_list[field_list_[i].field_name]["attribute"] = pybind11::make_tuple(
        field_list_[i].field_unit, py_name_alias, field_list_[i].field_display_name);
    py_field_list[field_list_[i].field_name]["field_type"] = field_list_[i].field_type;
    py_field_list[field_list_[i].field_name]["contiguous_in_x"] =
        pybind11::bool_(field_list_[i].contiguous_in_x);
    py_field_list[field_list_[i].field_name]["ghost_cell"] =
        pybind11::make_tuple(field_list_[i].field_ghost_cell[0],
                             field_list_[i].field_ghost_cell[1],
                             field_list_[i].field_ghost_cell[2],
                             field_list_[i].field_ghost_cell[3],
                             field_list_[i].field_ghost_cell[4],
                             field_list_[i].field_ghost_cell[5]);
  }
#endif  // #ifndef USE_PYBIND11
...
//              |                 |               |                |              |
//              { <par_type>: { "attribute" : { <attr_name1> : ( <attr_unit>,
//              (<attr_name_alias>), <attr_display_name>),
//                                              <attr_name2> : ( <attr_unit>,
//                                              (<attr_name_alias>),
//                                              <attr_display_name>)},
...
//               }
//-------------------------------------------------------------------------------------------------------
DataStructureOutput DataStructureAmr::BindParticleListToPython(
    PyObject* py_dict, const std::string& py_dict_name) const {
  if (check_data_) {
  ...
  for (int s = 0; s < num_par_types_; s++) {
    ...
    for (int a = 0; a < particle_list_[s].num_attr; a++) {
      ...
      if (PyList_Append(attr_list, val) != 0) {
        ...
        Py_XDECREF(val);
        std::string error =
            "(par_type, attr_unit) = (" + std::string(particle_list_[s].par_type) + ", " +
      ...
      for (int i = 0; i < attr.num_attr_name_alias; i++) {
        ...
        if (PyList_Append(name_alias_list, val) != 0) {
          ...
          std::string error = "(par_type, attr_name, attr_name_alias) = (" +
                              std::string(particle_list_[s].par_type) + ", " +
      ...
      if (PyList_Append(attr_list, name_alias_list) != 0) {
        ...
        Py_DECREF(attr_list);
        Py_DECREF(name_alias_list);
        std::string error = "(par_type, attr_name) = (" +
                            std::string(particle_list_[s].par_type) + ", " +
                            std::string(attr.attr_name) +
                            "), failed to append name_alias_list to list!\n";
        return {DataStructureStatus::kDataStructureFailed, error};
      }
      Py_DECREF(name_alias_list);
      ...
      if (PyDict_SetItem(attr_dict, key, attr_list) != 0) {
        ...
        Py_XDECREF(key);
        std::string error =
            "(par_type, attr_name) = (" + std::string(particle_list_[s].par_type) + ", " +
    ...
    if (PyDict_SetItemString(species_dict, "attribute", attr_dict) != 0) {
      ...
      Py_DECREF(attr_dict);
      std::string error =
          "(par_type) = (" + std::string(particle_list_[s].par_type) +
    ...
    if (particle_list_[s].coor_x == nullptr) {
      if (PyList_Append(coor_list, Py_None) != 0) {
        ...
        Py_DECREF(species_dict);
        Py_DECREF(coor_list);
        std::string error = "(par_type, coor_x) = (" +
                            std::string(particle_list_[s].par_type) +
                            ", nullptr), failed to append Py_None to list!\n";
        return {DataStructureStatus::kDataStructureFailed, error};
      }
      ...
      if (PyList_Append(coor_list, val) != 0) {
        ...
        std::string error = "(par_type, coor_x) = (" +
                            std::string(particle_list_[s].par_type) + ", " +
                            std::string(particle_list_[s].coor_x) +
                            "), failed to append coor_x to list!\n";
        return {DataStructureStatus::kDataStructureFailed, error};
      }
      Py_DECREF(val);
    }
    ...
    if (particle_list_[s].coor_y == nullptr) {
      if (PyList_Append(coor_list, Py_None) != 0) {
        ...
        Py_DECREF(species_dict);
        Py_DECREF(coor_list);
        std::string error = "(par_type, coor_y) = (" +
                            std::string(particle_list_[s].par_type) +
                            ", nullptr), failed to append Py_None to list!\n";
        return {DataStructureStatus::kDataStructureFailed, error};
      }
      ...
      if (PyList_Append(coor_list, val) != 0) {
        ...
        std::string error = "(par_type, coor_y) = (" +
                            std::string(particle_list_[s].par_type) + ", " +
                            std::string(particle_list_[s].coor_y) +
                            "), failed to append coor_y to list!\n";
        return {DataStructureStatus::kDataStructureFailed, error};
      }
      Py_DECREF(val);
    }
    ...
    if (particle_list_[s].coor_z == nullptr) {
      if (PyList_Append(coor_list, Py_None) != 0) {
        ...
        Py_DECREF(species_dict);
        Py_DECREF(coor_list);
        std::string error = "(par_type, coor_z) = (" +
                            std::string(particle_list_[s].par_type) +
                            ", nullptr), failed to append Py_None to list!\n";
        return {DataStructureStatus::kDataStructureFailed, error};
      }
      ...
      if (PyList_Append(coor_list, val) != 0) {
        ...
        std::string error = "(par_type, coor_z) = (" +
                            std::string(particle_list_[s].par_type) + ", " +
                            std::string(particle_list_[s].coor_z) +
                            "), failed to append coor_z to list!\n";
        return {DataStructureStatus::kDataStructureFailed, error};
      }
      Py_DECREF(val);
    }
    ...
    if (PyDict_SetItemString(species_dict, "label", key) != 0) {
      ...
      std::string error = "(par_type) = (" + std::string(particle_list_[s].par_type) +
                          "), failed to add key-value pair 'label' and " +
                          std::to_string(s) + "\n";
      return {DataStructureStatus::kDataStructureFailed, error};
    }
    Py_DECREF(key);
  ...
  if (PyDict_SetItemString(py_dict, "particle_list", particle_list_dict) != 0) {
    Py_DECREF(particle_list_dict);
    std::string error =
        "Inserting dictionary 'particle_list' to '" + py_dict_name + "' failed!\n";
    return {DataStructureStatus::kDataStructureFailed, error};
  }
  Py_DECREF(particle_list_dict);
#else
  ...
  for (int i = 0; i < num_par_types_; i++) {
    ...
    for (int v = 0; v < particle_list_[i].num_attr; v++) {
      pybind11::tuple py_name_alias =
          pybind11::tuple(particle_list_[i].attr_list[v].num_attr_name_alias);
      ...
      py_attr_dict[particle_list_[i].attr_list[v].attr_name] =
          pybind11::make_tuple(particle_list_[i].attr_list[v].attr_unit,
                               py_name_alias,
    ...
    py_particle_list[particle_list_[i].par_type]["particle_coor_label"] =
        pybind11::make_tuple(
            particle_list_[i].coor_x, particle_list_[i].coor_y, particle_list_[i].coor_z);
...
//-------------------------------------------------------------------------------------------------------
DataStructureOutput DataStructureAmr::BindInfoToPython(const std::string& py_dict_name,
                                                       PyObject* py_dict) {
  if (num_fields_ > 0) {
...
//                2. If it is under Mpi mode, we need to gather hierarchy from different
//                ranks to all ranks.
...
//                5. TODO: Do I need to move data twice, which is gathering data, and then
//                move it to Python
//                         storage?
//-------------------------------------------------------------------------------------------------------
...
//                5. TODO: Currently, the API forces this function to bind and build all
//                the data
...
DataStructureOutput DataStructureAmr::BindLocalFieldDataToPython(
  ...
  for (int v = 0; v < num_fields_; v++) {
    ...
    // Only "cell-centered" will be set to grid_dimensions + ghost cell, else should be
    // set in data_dimensions.
    ...
    for (int d = 0; d < dimensionality_; d++) {
      if ((grid.field_data)[v].data_dimensions[d] <= 0) {
        ...
        Py_DECREF(py_field_labels);
        std::string error = "(grid id, field) = (" + std::to_string(grid.id) + ", " +
                            std::string(field_list_[v].field_name) + ") data dimension " +
                            std::to_string(d) + " is " +
                            std::to_string((grid.field_data)[v].data_dimensions[d]) +
                            " <= 0.";
        return {DataStructureStatus::kDataStructureFailed, error};
      }
    }
...
//                         inside the grids_local_ array at once. Might change it in the
//                         future libyt v1.0.
//                5. TODO: Future Api shouldn't make hierarchy and data to closely
//                related, so that we can have
//                         more flexibility. Like Enzo contains particle data with tuple
//                         values.
//-------------------------------------------------------------------------------------------------------
DataStructureOutput DataStructureAmr::BindLocalParticleDataToPython(
    const yt_grid& grid) const {
  ...
  for (int p = 0; p < num_par_types_; p++) {
    ...
    for (int a = 0; a < particle_list_[p].num_attr; a++) {
      ...
      if (dtype_utilities::YtDtype2NumPyDtype(particle_list_[p].attr_list[a].attr_dtype) <
          0) {
        Py_DECREF(py_attributes);
        Py_DECREF(py_ptype_labels);
        Py_DECREF(py_grid_id);
        std::string error = "(particle type, attribute) = (" +
                            std::string(particle_list_[p].par_type) + ", " +
      ...
      if ((grid.par_count_list)[p] <= 0) {
        ...
        Py_DECREF(py_ptype_labels);
        Py_DECREF(py_grid_id);
        std::string error = "(particle type, grid id) = (" +
                            std::string(particle_list_[p].par_type) + ", " +
      ...
      py_data =
          numpy_controller::ArrayToNumPyArray(1,
                                              array_dims,
                                              particle_list_[p].attr_list[a].attr_dtype,
                                              (grid.particle_data)[p][a].data_ptr,
                                              true,
                                              false);
      ...
      if (PyDict_Contains(py_particle_data_, py_grid_id) != 1) {
        ...
        PyDict_SetItem(py_particle_data_,
                       py_grid_id,
        ...
        if (PyDict_Contains(py_ptype_labels, py_ptype_name) != 1) {
          PyDict_SetItemString(
              py_ptype_labels, particle_list_[p].par_type, py_attributes);
        }
        Py_DECREF(py_ptype_name);
      }
      PyDict_SetItemString(
          py_attributes, particle_list_[p].attr_list[a].attr_name, py_data);
      Py_DECREF(py_data);
    }
    Py_DECREF(py_attributes);
  }
...
//                2. The current structure will not know if the data is set or not.
//                3. TODO: Currently, the API forces this function to bind and build all
//                the data
//                         inside the grids_local_ array at once. Might change it in the
//                         future libyt v1.0.
//-------------------------------------------------------------------------------------------------------
...
//
// Notes       :  1. Clean up grids_local_, since it's only for user to pass in hierarchy
// and data.
//                2. Reset
//                num_grids_local_/num_grids_local_field_data_/num_grids_local_par_data_ =
//                0 and
//                   grids_local_ = nullptr.
//                3. Counterpart of AllocateGridsLocal().
//                4. This method is separate from the rest of the clean up methods is
//                because it cleans up
//                   the data for holding user input, which is not needed after committing
//                   everything.
//                   TODO: bad Api design
//-------------------------------------------------------------------------------------------------------
...
//
// Notes       :  1. Clean full hierarchy Python bindings, and reset hierarchy pointer to
// nullptr and num_grids_ = 0.
...
//
// Notes       :  1. Get the particle attribute index in the particle_list_ based on
// particle index and
//                   attribute name.
//                2. Return -1 if the particle_type or attribute name is not found.
//-------------------------------------------------------------------------------------------------------
int DataStructureAmr::GetParticleAttributeIndex(int particle_type_index,
  ...
  for (int a = 0; a < particle_list_[particle_type_index].num_attr; a++) {
    if (strcmp(attr_name, particle_list_[particle_type_index].attr_list[a].attr_name) ==
        0) {
      pattr_index = a;
      break;
    }
...
DataStructureOutput DataStructureAmr::GenerateLocalFieldData(
  ...
  for (const long& kGid : gid_list) {
    ...
    if (proc_num != DataStructureAmr::mpi_rank_) {
      std::string error = std::string("Grid id [ ") + std::to_string(kGid) +
                          std::string(" ] is not local.\n");
      return {DataStructureStatus::kDataStructureFailed, error};
    }
    ...
    int grid_dim[3];
    DataStructureOutput status =
        GetPythonBoundFullHierarchyGridDimensions(kGid, &grid_dim[0]);
    if (status.status != DataStructureStatus::kDataStructureSuccess) {
      std::string error = status.error;
      error += std::string("Failed to get grid dim for (field_name, gid) = (") +
               field_name + std::string(", ") + std::to_string(kGid) +
    ...
    for (int d = 0; d < 3; d++) {
      if (grid_dim[d] <= 0) {
        std::string error =
            std::string("Grid dim for (field_name, gid) = (") + field_name +
    ...
    if (amr_data.data_ptr == nullptr) {
      std::string error =
          std::string("Failed to allocate memory for (field_name, gid) = (") +
          field_name + std::string(", ") + std::to_string(kGid) +
...
//                2. Append the generated data in storage. If the length is 0, it will
//                still append it.
//                3. Allocate new memory, and it is the callers responsibility to free it.
//                4. TODO: Consider OpenMP, same question in field derived function.
//-------------------------------------------------------------------------------------------------------
DataStructureOutput DataStructureAmr::GenerateLocalParticleData(
  ...
  if (ptype_index == -1 || pattr_index == -1) {
    std::string error = std::string("(particle type, attribute) = (") + ptype +
                        std::string(", ") + attr + std::string(") not found.\n");
    return {DataStructureStatus::kDataStructureFailed, error};
  }
  ...
  for (const long& kGid : gid_list) {
    ...
    if (ds_status.status != DataStructureStatus::kDataStructureSuccess) {
      ...
      error += std::string("Failed to get particle count for (particle type, gid) = (") +
               ptype + std::string(", ") + std::to_string(kGid) +
               std::string(") on MPI rank ") +
               std::to_string(DataStructureAmr::mpi_rank_) + std::string(".\n");
      return {DataStructureStatus::kDataStructureFailed, error};
    }
    ...
    if (get_par_attr == nullptr) {
      std::string error =
          std::string("Get particle function get_par_attr not set in particle type [ ") +
...
//                2. Counterpart of BindAllHierarchyToPython().
//-------------------------------------------------------------------------------------------------------
DataStructureOutput DataStructureAmr::GetPythonBoundFullHierarchyGridParentId(
    long gid, long* parent_id) const {
...
//                2. Counterpart of BindAllHierarchyToPython().
//-------------------------------------------------------------------------------------------------------
DataStructureOutput DataStructureAmr::GetPythonBoundFullHierarchyGridLevel(
    long gid, int* level) const {
...
//                2. Counterpart of BindAllHierarchyToPython().
//-------------------------------------------------------------------------------------------------------
DataStructureOutput DataStructureAmr::GetPythonBoundFullHierarchyGridProcNum(
    long gid, int* proc_num) const {
...
//
// Notes       :  1. Read the full hierarchy grid particle count for a ptype loaded in
// Python.
//                2. This method is only valid if the data structure contains particle
//                data.
//                3. Counterpart of BindAllHierarchyToPython().
//-------------------------------------------------------------------------------------------------------
DataStructureOutput DataStructureAmr::GetPythonBoundFullHierarchyGridParticleCount(
    long gid, const char* ptype, long* par_count) const {
  if (!has_particle_) {
  ...
  if (label == -1) {
    std::string error =
        "Cannot find (particle type) = " + std::string(ptype) + " in particle_list.\n";
    return {DataStructureStatus::kDataStructureFailed, error};
  }
...
DataStructureOutput DataStructureAmr::GetPythonBoundLocalFieldData(
  ...
  if (PyDict_Contains(py_grid_data_, py_grid_id) != 1 ||
    ...
    std::string error =
        "Cannot find field data (grid id, field) = " + std::to_string(gid) + ", " +
        field_name + " on MPI rank " + std::to_string(mpi_rank_) + ".\n";
    Py_DECREF(py_grid_id);
    Py_DECREF(py_field);
    return {DataStructureStatus::kDataStructureFailed, error};
  }
...
//
// Notes       :  1. Read the local field data bind to Python
// libyt.particle_data[gid][ptype][attr].
//                2. Counterpart of BindLocalParticleDataToPython().
//-------------------------------------------------------------------------------------------------------
DataStructureOutput DataStructureAmr::GetPythonBoundLocalParticleData(
    long gid, const char* ptype, const char* attr, yt_data* par_data) const {
  ...
  if (PyDict_Contains(py_particle_data_, py_grid_id) != 1 ||
      ...
      PyDict_Contains(
          PyDict_GetItem(PyDict_GetItem(py_particle_data_, py_grid_id), py_ptype),
          py_attr) != 1) {
    Py_DECREF(py_grid_id);
    Py_DECREF(py_ptype);
    Py_DECREF(py_attr);
    ...
    std::string error =
        "Cannot find particle data (grid id, particle type, attribute) = " +
        std::to_string(gid) + ", " + ptype + ", " + attr + " on MPI rank " +
  ...
  // Get NumPy array dimensions/data pointer/dtype
  NumPyArray py_data_info = numpy_controller::GetNumPyArrayInfo(PyDict_GetItem(
...
//                   (1) Check every grid id are unique. (Be careful that it can be
//                   non-0-indexing.) (2) Check if all grids with level > 0, have a good
//                   parent id. (3) Check if children grids' edge fall between parent's.
//                   (4) Check parent's level = children level - 1.
//                2. Note that the method's signature has different input parameters for
//                Mpi and SERIAL_MODE:
//                   yt_hierarchy* (for MPI) and yt_grid* (for SERIAL_MODE).
//                   TODO: we can merge them when updating the API libyt-v1.0
//-------------------------------------------------------------------------------------------------------
#ifndef SERIAL_MODE
DataStructureOutput DataStructureAmr::CheckHierarchyIsValid(
    yt_hierarchy* hierarchy) const {
#else
DataStructureOutput DataStructureAmr::CheckHierarchyIsValid(yt_grid* hierarchy) const {
  ...
  for (long i = 0; i < num_grids_; i++) {
    if (order[hierarchy[i].id - index_offset_] == -1) {
      ...
      delete[] order;
      std::string error = "(grid id) = " + std::to_string(hierarchy[i].id) +
                          " are not unique, both MPI rank " +
                          std::to_string(hierarchy[i].proc_num) + " and " +
                          std::to_string(other_proc_num) + " have this grid id!\n";
      return {DataStructureStatus::kDataStructureFailed, error};
    }
  }

  // Check if all level > 0 have good parent id, and that children's edges don't exceed
  // parent's
  std::string error;
  for (long i = 0; i < num_grids_; i++) {
    if (hierarchy[i].level > 0) {
      ...
      if ((hierarchy[i].parent_id - index_offset_ < 0) ||
          hierarchy[i].parent_id - index_offset_ >= num_grids_) {
        error = "(grid id, level, parent id) = (" + std::to_string(hierarchy[i].id) +
                ", " + std::to_string(hierarchy[i].level) + ", " +
                std::to_string(hierarchy[i].parent_id) +
                "), parent id is out of range, expect to be between " +
                std::to_string(index_offset_) + " ~ " +
                std::to_string(num_grids_ + index_offset_ - 1) + ".\n";
        break;
      } else {
        // Check children's edges fall between parent's
        double* parent_left_edge =
            hierarchy[order[hierarchy[i].parent_id - index_offset_]].left_edge;
        double* parent_right_edge =
            hierarchy[order[hierarchy[i].parent_id - index_offset_]].right_edge;
        for (int d = 0; d < 3; d++) {
        ...
        if (parent_level != hierarchy[i].level - 1) {
          error = "(grid id, parent id) = (" + std::to_string(hierarchy[i].id) + ", " +
                  std::to_string(hierarchy[i].parent_id) + "), parent level " +
                  std::to_string(parent_level) + " != children level " +
                  std::to_string(hierarchy[i].level) + " - 1.\n";
          break;
        }
      }
    }
...
DataStructureOutput DataStructureAmr::CheckFieldList() const {
  ...
  for (int v = 0; v < num_fields_; v++) {
    ...
    if (status.status != DataStructureStatus::kDataStructureSuccess) {
      status.error +=
          "(field_name) = " + std::string(field.field_name) + " is not valid.\n";
      return status;
    }
  }
  ...
  for (int v1 = 0; v1 < num_fields_; v1++) {
    for (int v2 = v1 + 1; v2 < num_fields_; v2++) {
      if (strcmp(field_list_[v1].field_name, field_list_[v2].field_name) == 0) {
        std::string error = "field_name in field_list[" + std::to_string(v1) +
                            "] and field_list[" + std::to_string(v2) +
                            "] are not unique!\n";
        return {DataStructureStatus::kDataStructureFailed, error};
      }
    }
...
//                   (2) field_type can only be : "cell-centered", "face-centered",
//                   "derived_func". (3) Check if field_dtype is set. (4) Raise warning if
//                   derived_func == NULL and field_type is set to "derived_func". (5)
//                   field_ghost_cell cannot be smaller than 0.
//                2. The fact that I'm defining how to check field here is weird, maybe
//                should put it in
//                   field class or something. Since yt_field is meant to be a struct
...
DataStructureOutput DataStructureAmr::CheckField(const yt_field& field) const {
  ...
  if (!check2) {
    std::string error =
        "(field_name) = " + std::string(field.field_name) + ", field_dtype is not set.\n";
    return {DataStructureStatus::kDataStructureFailed, error};
  }
  ...
  for (int d = 0; d < 6; d++) {
    if (field.field_ghost_cell[d] < 0) {
      std::string error =
          "(field_name) = (" + std::string(field.field_name) + "), field_ghost_cell[" +
          std::to_string(d) +
...
//                   frontend. (NOT CHECK) (3) Species names (or ptype in YT-term) are all
//                   unique.
//-------------------------------------------------------------------------------------------------------
DataStructureOutput DataStructureAmr::CheckParticleList() const {
  ...
  for (int p = 0; p < num_par_types_; p++) {
    ...
    if (status.status != DataStructureStatus::kDataStructureSuccess) {
      status.error +=
          "(particle type) = (" + std::string(particle.par_type) + ") is not valid!\n";
      return status;
    }
  }
  ...
  for (int p1 = 0; p1 < num_par_types_; p1++) {
    for (int p2 = p1 + 1; p2 < num_par_types_; p2++) {
      if (strcmp(particle_list_[p1].par_type, particle_list_[p2].par_type) == 0) {
        std::string error = "par_type in particle_list[" + std::to_string(p1) +
                            "] and particle_list[" + std::to_string(p2) +
                            "] are the same, par_type should be unique!\n";
        return {DataStructureStatus::kDataStructureFailed, error};
      }
    }
  }
...
//                2. The fact that I'm defining how to check particle here is weird, maybe
//                should put it in
//                   particle class or something. Since yt_particle is meant to be a
//                   particle only, I'm keeping this.
...
DataStructureOutput DataStructureAmr::CheckParticle(yt_particle& particle) const {
  ...
  for (int i = 0; i < particle.num_attr; i++) {
    ...
    if (status.status != DataStructureStatus::kDataStructureSuccess) {
      status.error += "(particle type) = (" + std::string(particle.par_type) +
                      "), attr_list element [" + std::to_string(i) + "] is not valid!\n";
      return status;
    }
  }
  ...
  for (int i = 0; i < particle.num_attr; i++) {
    for (int j = i + 1; j < particle.num_attr; j++) {
      if (strcmp(particle.attr_list[i].attr_name, particle.attr_list[j].attr_name) == 0) {
        std::string error = "(particle type) = (" + std::string(particle.par_type) +
                            "), attr_list element [" + std::to_string(i) + "] and [" +
                            std::to_string(j) +
  ...
  if (particle.coor_x == nullptr) {
    std::string error =
        "(particle type) = (" + std::string(particle.par_type) + "), coor_x not set!\n";
    return {DataStructureStatus::kDataStructureFailed, error};
  }
  if (particle.coor_y == nullptr) {
    std::string error =
        "(particle type) = (" + std::string(particle.par_type) + "), coor_y not set!\n";
    return {DataStructureStatus::kDataStructureFailed, error};
  }
  if (particle.coor_z == nullptr) {
    std::string error =
        "(particle type) = (" + std::string(particle.par_type) + "), coor_z not set!\n";
    return {DataStructureStatus::kDataStructureFailed, error};
  }
...
//                2. The fact that I'm defining how to check particle attribute here is
//                weird, maybe should put it in
//                   particle class or something. Since yt_attribute is meant to be for
//                   particle only, I'm keeping this.
...
DataStructureOutput DataStructureAmr::CheckParticleAttribute(yt_attribute& attr) const {
  ...
  if (!valid) {
    std::string error =
        "(attr_name) = (" + std::string(attr.attr_name) + "), unknown attr_dtype!\n";
    return {DataStructureStatus::kDataStructureFailed, error};
  }
...
//                   (6) grid left edge <= grid right edge. (Not sure if this still holds
//                   for periodic condition.) (7) Abort if field_type = "cell-centered",
//                   and data_ptr == NULL. (8) Abort if field_type = "face-centered", and
//                   data_ptr == NULL. (9) If data_ptr != NULL, then data_dimensions > 0
...
DataStructureOutput DataStructureAmr::CheckGridsLocal() const {
  ...
  for (int i = 0; i < num_grids_local_; i++) {
    ...
    // (2) parent ID is inside range if there is one (level > 0).
    if ((grid.level > 0) && (grid.parent_id - index_offset_ >= num_grids_ ||
                             grid.parent_id - index_offset_ < 0)) {
      std::string error = "(grid id, level, parent id) = (" + std::to_string(grid.id) +
                          ", " + std::to_string(grid.level) + ", " +
                          std::to_string(grid.parent_id) +
                          "), parent id is out of range!\n";
      return {DataStructureStatus::kDataStructureFailed, error};
    }

    // (3) Root level starts at 0. So if level == 0, which has no parent, then parent ID
    // >= 0 is error.
    if (grid.level < 0) {
      std::string error = "(grid id, level) = (" + std::to_string(grid.id) + ", " +
                          std::to_string(grid.level) + "), level < 0 is not valid!\n";
      return {DataStructureStatus::kDataStructureFailed, error};
    }
    ...
    for (int d = 0; d < 3; d = d + 1) {
      ...
      if (grid.right_edge[d] < grid.left_edge[d]) {
        std::string error = "(grid id) = (" + std::to_string(grid.id) + "), dim " +
                            std::to_string(d) + " has right edge < left edge!\n";
        return {DataStructureStatus::kDataStructureFailed, error};
      }
    }
    ...
    for (int v = 0; v < num_fields_; v = v + 1) {
      if (strcmp(field_list_[v].field_type, "cell-centered") == 0) {
        // (7) Raise error if field_type = "cell-centered", and data_ptr is not set ==
        // NULL.
        if (grid.field_data[v].data_ptr == nullptr) {
          std::string error =
              "(grid id, field_name, field_type) = (" + std::to_string(grid.id) + ", " +
              std::string(field_list_[v].field_name) + ", " +
              std::string(field_list_[v].field_type) + "), data is nullptr!\n";
          return {DataStructureStatus::kDataStructureFailed, error};
        }
      } else if (strcmp(field_list_[v].field_type, "face-centered") == 0) {
        // (8) Raise error if field_type = "face-centered", and data_ptr is not set ==
        // NULL.
        if (grid.field_data[v].data_ptr == nullptr) {
          std::string error =
              "(grid id, field_name, field_type) = (" + std::to_string(grid.id) + ", " +
...
//                2. The fact that I'm defining how to check grid here is weird, maybe
//                should put it in
//                   grid class or something. Since yt_grid is meant to be struct only,
//                   I'm keeping this.
...
DataStructureOutput DataStructureAmr::CheckGrid(yt_grid& grid) const {
  ...
  for (int d = 0; d < 3; d++) {
    if (grid.left_edge[d] == DBL_UNDEFINED) {
      std::string error = "(grid id) = (" + std::to_string(grid.id) + "), left_edge[" +
                          std::to_string(d) + "] is not set!\n";
      return {DataStructureStatus::kDataStructureFailed, error};
    }
    if (grid.right_edge[d] == DBL_UNDEFINED) {
      std::string error = "(grid id) = (" + std::to_string(grid.id) + "), right_edge[" +
                          std::to_string(d) + "] is not set!\n";
      return {DataStructureStatus::kDataStructureFailed, error};
    }
  }
  ...
  for (int d = 0; d < 3; d++) {
    if (grid.grid_dimensions[d] <= 0) {
      std::string error = "(grid id) = (" + std::to_string(grid.id) +
                          "), grid_dimensions[" + std::to_string(d) +
  ...
  if (grid.id - index_offset_ >= num_grids_ || grid.id - index_offset_ < 0) {
    std::string error =
        "(grid id) = (" + std::to_string(grid.id) + "), grid id is out of range!\n";
    return {DataStructureStatus::kDataStructureFailed, error};
  }
  ...
  if (grid.parent_id == LNG_UNDEFINED) {
    std::string error =
        "(grid id) = (" + std::to_string(grid.id) + "), parent id is not set!\n";
    return {DataStructureStatus::kDataStructureFailed, error};
  }
  ...
  if (grid.proc_num < 0 || grid.proc_num >= mpi_size_) {
    std::string error = "(grid id) = (" + std::to_string(grid.id) +
                        "), proc_num (MPI rank) is out of range!\n";
    return {DataStructureStatus::kDataStructureFailed, error};
  }

...
DataHubReturn<AmrDataArray1D> DataHubAmrDataArray1D::GetLocalParticleData(const DataStructureAmr& ds_amr,
    ...
    if (ptype_index == -1 || pattr_index == -1) {
        error_str_ = std::string("(particle type, attribute) = (") + ptype + std::string(", ") + pattr +
                     std::string(") not found.\n");
        return {DataHubStatus::kDataHubFailed, data_array_list_};
    }

    // Loop through grid_id_list and try to get data in libyt.particle_data
    // If failed, which means the length may be 0 or it needs to call get_par_attr (TODO: should I separate 0 length?)
    ...
    for (const long& kGid : grid_id_list) {
        ...
        DataStructureOutput status =
            ds_amr.GetPythonBoundLocalParticleData(kGid, ptype.c_str(), pattr.c_str(), &par_array);
        if (status.status == DataStructureStatus::kDataStructureSuccess) {

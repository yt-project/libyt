...
void CommMpi::SetStringUsingValueOnRank(std::string& sync_string, int src_mpi_rank) {
  ...
  if (mpi_rank_ == src_mpi_rank) {
    MPI_Bcast((void*)sync_string.c_str(),
              (int)sync_string_len,
              MPI_CHAR,
              src_mpi_rank,
              MPI_COMM_WORLD);
    ...
    MPI_Bcast(
        (void*)dest_string, (int)sync_string_len, MPI_CHAR, src_mpi_rank, MPI_COMM_WORLD);
...
void CommMpi::GatherAllStringsToRank(std::vector<std::string>& all_strings,
  ...
  if (mpi_rank_ == dest_mpi_rank) {
    ...
    all_strings.clear();
    for (int r = 0; r < mpi_size_; r++) {
      all_strings.emplace_back(
          std::string(&buffer[displacements[r]], all_src_string_len[r]));
    }
    ...
    MPI_Gather(
        &src_string_len, 1, MPI_INT, nullptr, 0, MPI_INT, dest_mpi_rank, MPI_COMM_WORLD);
    MPI_Gatherv(src_string.c_str(),
                src_string_len,
                MPI_CHAR,
                nullptr,
...
void CommMpi::GatherAllIntsToRank(std::vector<int>& all_ints, const int src_int,
  ...
  MPI_Gather(
      &src_int, 1, MPI_INT, all_ints.data(), 1, MPI_INT, dest_mpi_rank, MPI_COMM_WORLD);
}

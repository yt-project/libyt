...
static std::vector<std::string> GenerateErrMsg(
    const std::vector<std::string>& statements);
static bool LastLineHasBackslash(const std::string& code);
static bool LastLineHasColon(const std::string& code);
static void SplitOnLine(const std::string& code, unsigned int lineno,
                        std::array<std::string, 2>& code_split);
...
//
// Notes       :  1. Only root rank will record all the successful inputs from interactive
// prompt.
//                2. Input that failed will not be recorded.
//                3. Inputs will not accumulate from iterations to iterations, which means
//                input will be
...
int LibytPythonShell::UpdateHistory(const std::string& cmd_prompt) {
  ...
  history_ = history_ + std::string("#In[") + std::to_string(history_count_) +
             std::string("]\n");
...
//                  3. Get only keyword def defined functions. If the functors are defined
//                  using __call__
//                     this method cannot grab the corresponding definition.
//                  4. TODO: It needs script's scope, otherwise some functors aren't
//                  detectable.
//                  5. TODO: Re-write this using C API instead of PyRun_SimpleString, and
//                  make it store
//                           the function body under some PyObject* dictionary in the
//                           class data member.
//
// Arguments     :  const char *filename: update function body for function defined inside
// filename
//
...
int LibytPythonShell::load_file_func_body(const char* filename) {
  ...
  sprintf(command,
          ...
          "        try:\n"
          "            if callable(var) and inspect.getsourcefile(var).split(\"/\")[-1] "
          "== \"%s\":\n"
  ...
  if (PyRun_SimpleString(command) == 0) {
    logging::LogDebug("Loading function body in script %s ... done\n", filename);
    free(command);
    return YT_SUCCESS;
  } else {
    logging::LogDebug("Loading function body in script %s ... failed\n", filename);
    free(command);
    return YT_FAIL;
  }
}
...
//
// Notes         :  1. This is a static method.
//                  2. Detect if there are functors defined in code object src_ptr, if
//                  yes, put it under
//                     libyt.interactive_mode["func_body"].
//                  3. Every MPI process has a copy of function body.
//                  4. It's not this method's responsibility to free code.
//                  5. To silent the printing when PyEval_EvalCode evaluates the code,
//                  that sys.stdout
//                     point to somewhere else when evaluating.
//                  6. It accepts indent size different from 4.
//                  7. TODO: (Unit Test) It probably needs a mock for
//                  LibytProcessControl::Get().function_info_list_
//                  7. TODO: It needs script's scope, otherwise some functors aren't
//                  detectable.
//                     (ex: b = np.random.rand)
//
...
int LibytPythonShell::load_input_func_body(const char* code) {
  ...
  PyObject* py_src =
      Py_CompileString(command_str.c_str(), "<libyt-stdin>", Py_file_input);
  if (py_src != NULL) {
    ...
    for (Py_ssize_t i = 0; i < py_size; i++) {
      if (PyCallable_Check(
              PyDict_GetItem(py_new_dict, PyList_GET_ITEM(py_new_dict_keys, i)))) {
        // add new function to LibytProcessControl::Get().function_info_list_ and set to
        // idle. if function exists already, get its index
        const char* func_name = PyUnicode_AsUTF8(PyList_GET_ITEM(py_new_dict_keys, i));
        LibytProcessControl::Get().function_info_list_.AddNewFunction(
            func_name, FunctionInfo::RunStatus::kWillIdle);
...
//
// Notes         :  1. This is a static method.
//                  2. It grabs functions or any callable object's name defined in
//                  filename.
//                  3. TODO: Re-write this using C API instead of PyRun_SimpleString, and
//                  make it store
//                           the function body under some PyObject* dictionary in the
//                           class data member.
//
...
// filename
//
// Return        : std::vector<std::string> contains a list of function name defined in
// filename.
//-------------------------------------------------------------------------------------------------------
std::vector<std::string> LibytPythonShell::get_funcname_defined(const char* filename) {
  ...
  sprintf(command,
          ...
          filename);
  if (PyRun_SimpleString(command) != 0)
    logging::LogError("Unable to grab functions in python script %s.\n", filename);

  PyObject* py_func_list =
      PyDict_GetItemString(LibytProcessControl::Get().py_interactive_mode_, "temp");
...
//
// Notes         :  1. This is a static method.
//                  2. Makes MPI not invoke MPI_Abort when getting errors in Python in
//                  interactive mode.
...
int LibytPythonShell::SetExceptionHook() {
  ...
  sprintf(
      ...
      "    print(\"[YT_ERROR  ] {}: {}\".format(exception_type.__name__, "
      "exception_value), file=sys.stderr)\n"
...
//
// Notes         :  1. This is a static method.
//                  2. The not-done-error message initialized are those that can span
//                  multi-line.
...
//                       (7)  with
//                       (8)  match / match-case
//                     We will drop every string after first number occurs in the error
//                     message, since some of them will print the lineno, and it is Python
//                     version dependent. Ex: "SyntaxError: unterminated triple-quoted
//                     string literal (detected at line 1)" This is put in unit test to
//                     make sure that the number appears within the last 3 chars of the
//                     error msg.
...
int LibytPythonShell::InitializeNotDoneErrMsg() {
  ...
  std::vector<std::string> bracket_statement = {std::string("\"\"\""),
                                                std::string("'''"),
                                                std::string("r\"\"\""),
                                                std::string("u\"\"\""),
                                                std::string("f\"\"\""),
  ...
  std::vector<std::string> compound_keyword = {
      std::string("if 1==1:"),
      std::string("if 1==1:\n  pass\nelse:"),
      std::string("if 1==1:\n  pass\nelif 2==2:"),
      std::string("try:"),
...
//
// Notes         :  1. This is a static method.
//                  2. Set the execution namespace for Python interpreter. The namespace
//                  is used to execute
//                     Python code.
...
//                     not done yet. The error msg is initialized in
//                     init_not_done_err_msg.
//                  3. If it is indeed caused by user not done its input, clear error
//                  buffer. If not,
//                     restore the buffer and let others deal with the error msg.
//                  4. The rule to check if it is a real error or not-yet-done is in this
//                  order:
//                     (1) If last line of the code is ended by '\', it is not-yet-done.
//                     (2) If match error msg of s_CompoundKeyword_NotDoneErr:
//                         (2)-1 and if colon exist at last line and err lineno at the
//                         last line,
//                               then it's a user not-yet-done.
//                     (3) If the error is caused by bracket not closing
//                     s_Bracket_NotDoneErr,
...
bool LibytPythonShell::IsNotDoneErrMsg(const std::string& code) {
  ...
  if (match_compoundkeyword_errmsg && LastLineHasColon(code) &&
      err_lineno == line_count) {
    user_not_done = true;
  }
...
//
// Notes       :  1. Test if it can compile based on Py_single_input (if in prompt env),
// otherwise compile
...
//-------------------------------------------------------------------------------------------------------
CodeValidity LibytPythonShell::CheckCodeValidity(const std::string& code, bool prompt_env,
...
// Description   :  Execute code on every MPI process based on Python input type.
// (collective operation)
//
// Notes       :  1. This is a collective operation, requires every rank to call this
// function.
//                2. To avoid copying data string back and forth, client will need to pass
//                in the designated
//                   output storage.
//                3. The local output are stored in the mpi rank order, which is
//                output[mpi_rank_], and the
//                   returned status represents all ranks successfully done or failed the
//                   job. Only the output_mpi_rank has full knowledge (status, output,
//                   error) of each rank, the others only knows itself and the overall
//                   status. output_mpi_rank should be the same in every rank.
//                4. To avoid unnecessary copying of code and cell name, they are stored
//                in different variables,
//                   ending with _sync, even though this will make the code less readable.
//                5. Python input type should be: Py_single_input (256), Py_file_input
//                (257), Py_eval_input (258)
//                   define in Python header. (TODO: should check in unit test)
//                6. This function doesn't check code validity. It's better to call
//                CheckCodeValidity before calling.
//                7. TODO: probably need to find another to redirect and capture the
//                stdout/stderr, or
//                         create a new class. also, the current method is probably not
//                         thread-safe.
//-------------------------------------------------------------------------------------------------------
PythonStatus LibytPythonShell::AllExecute(int python_input_type, const std::string& code,
  ...
  if (mpi_rank_ == src_rank) {
    ...
    CommMpi::SetStringUsingValueOnRank(const_cast<std::string&>(cell_base_name),
                                       src_rank);
  } else {
  ...
  if (strlen(code_ptr) <= 0) {
    ...
    if (mpi_rank_ == output_mpi_rank) {
      output.assign(mpi_size_,
                    PythonOutput{.status = PythonStatus::kPythonSuccess,
                                 .output = std::string(""),
                                 .error = std::string("")});
    } else {
      output.assign(mpi_size_,
                    PythonOutput{.status = PythonStatus::kPythonUnknown,
                                 .output = std::string(""),
                                 .error = std::string("")});
  ...
  if (py_src != NULL) {
    PyObject* py_dump =
        PyEval_EvalCode(py_src, GetExecutionNamespace(), GetExecutionNamespace());
    if (PyErr_Occurred()) {
      has_error = true;
      PyErr_Print();
  ...
  // Collect the local stdout/stderr and store in output vector, the elements are stored
  // in the same order as rank
  output.clear();
  output.assign(mpi_size_,
                PythonOutput{.status = PythonStatus::kPythonUnknown,
                             .output = std::string(""),
  ...
  CommMpi::GatherAllIntsToRank(
      all_status, static_cast<int>(output[mpi_rank_].status), output_mpi_rank);
  if (mpi_rank_ == output_mpi_rank) {
  ...
  CommMpi::GatherAllStringsToRank(
      all_gather_string, output[mpi_rank_].output, output_mpi_rank);
  if (mpi_rank_ == output_mpi_rank) {
    for (int i = 0; i < mpi_size_; i++) {
  ...
  CommMpi::GatherAllStringsToRank(
      all_gather_string, output[mpi_rank_].error, output_mpi_rank);
  if (mpi_rank_ == output_mpi_rank) {
    for (int i = 0; i < mpi_size_; i++) {
...
// Description   :  Execute a single statement code on every MPI process. (collective
// operation)
//
// Notes       :  1. Call AllExecute with python input type as Py_single_input (int).
//-------------------------------------------------------------------------------------------------------
PythonStatus LibytPythonShell::AllExecutePrompt(const std::string& code,
                                                const std::string& cell_base_name,
                                                int src_rank,
                                                std::vector<PythonOutput>& output,
                                                int output_mpi_rank) {
  SET_TIMER(__PRETTY_FUNCTION__);

  PythonStatus status = AllExecute(
      Py_single_input, code, cell_base_name, src_rank, output, output_mpi_rank);
  return status;
}
...
// Description   :  Execute an arbitrary length code on every MPI process. (collective
// operation)
//
// Notes       :  1. Call AllExecute with python input type as Py_file_input (int).
//-------------------------------------------------------------------------------------------------------
PythonStatus LibytPythonShell::AllExecuteFile(const std::string& code,
                                              const std::string& cell_base_name,
                                              int src_rank,
                                              std::vector<PythonOutput>& output,
                                              int output_mpi_rank) {
  ...
  PythonStatus status =
      AllExecute(Py_file_input, code, cell_base_name, src_rank, output, output_mpi_rank);
  return status;
}
...
// Description   :  Execute an arbitrary length code and display the output like a
// JupyterLab cell
//                  (collective operation)
//
// Notes       :  1. For the code execution to be like a JupyterLab cell, the last line of
// code should
//                   behave like AllExecutePrompt and the rest of the code should behave
//                   like AllExecuteFile.
//                2. Since only the src_rank will input the code, we do the parsing on the
//                src_rank only. 3  If error occurred in AllExecuteFile, it will skip
//                AllExecutePrompt and return the error.
//                   Otherwise, it will combine the output and error from AllExecuteFile
//                   and AllExecutePrompt.
//                4. AllExecute doesn't check validity, it just executes it. But this
//                method should be able to
//                   resolve an invalid code.
//-------------------------------------------------------------------------------------------------------
PythonStatus LibytPythonShell::AllExecuteCell(const std::string& code,
                                              const std::string& cell_base_name,
                                              int src_rank,
                                              std::vector<PythonOutput>& output,
                                              int output_mpi_rank) {
  ...
  if (mpi_rank_ == src_rank) {
    ...
    if (last_statement_lineno > 0) {
      ...
      // Add newline at the end of the last statement, so that Python won't produce EOF
      // error Add newline at the front of the last statement, so that Python error buffer
      // can catch the correct lineno.
  ...
  if (last_statement_lineno == 0) {
    ...
    if (mpi_rank_ == output_mpi_rank) {
      output.assign(mpi_size_,
                    PythonOutput{.status = PythonStatus::kPythonFailed,
                                 .output = std::string(""),
                                 .error = std::string("")});
    } else {
      output.assign(mpi_size_,
  ...
  PythonStatus status = AllExecute(
      Py_file_input, code_split[0], cell_base_name, src_rank, output, output_mpi_rank);
  if (status == PythonStatus::kPythonFailed) {
    return status;
  }

  std::vector<PythonOutput> output_prompt;
  status = AllExecute(Py_single_input,
                      code_split[1],
                      cell_base_name,
                      src_rank,
                      output_prompt,
...
//                3. Python lineno count starts at 1. So If lineno is 0, it means the code
//                is blank.
...
long LibytPythonShell::GetLastStatementLineno(const std::string& code) {
  ...
  if (py_result == NULL) {
    ...
    if (num_statements <= 0) {
      ...
      PyObject* py_lineno = PyObject_GetAttrString(
          PyList_GET_ITEM(py_result_body, num_statements - 1), "lineno");
...
//
// Return        :  std::vector<std::string> : a list of error msg, neglecting everything
// after numbers
//                                             if there is.
//-------------------------------------------------------------------------------------------------------
static std::vector<std::string> GenerateErrMsg(
...
//
// Notes         :  1. This function is used for distinguishing keywords for multi-line
// and the true error.
//                  2. An indentation error caused by user-not-done-yet will end with ':'
//                  in the last line.
//
...
//                3. code_split[0] contains line 1 ~ lineno, code_split[1] contains the
//                rest.
//-------------------------------------------------------------------------------------------------------
static void SplitOnLine(const std::string& code, unsigned int lineno,
                        std::array<std::string, 2>& code_split) {

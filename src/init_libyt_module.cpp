...
//                   (1) libyt.interactive_mode["script_globals"] =
//                   sys.modules["<script>"].__dict__ (2)
//                   libyt.interactive_mode["func_err_msg"] = dict()
//                4. Bind
//                py_grid_data/py_particle_data/py_hierarchy/py_param_yt/py_param_user/
...
int python_controller::PreparePythonEnvForLibyt() {
  ...
  LibytProcessControl::Get().data_structure_amr_.SetPythonBindings(
      libyt.attr("hierarchy").ptr(),
      libyt.attr("grid_data").ptr(),
      libyt.attr("particle_data").ptr());
  ...
  if (LibytProcessControl::Get().mpi_rank_ == 0) {
    std::string script_fullname =
        std::string(LibytProcessControl::Get().param_libyt_.script) + std::string(".py");
  ...
  std::string command_str =
      "import " + std::string(LibytProcessControl::Get().param_libyt_.script);
  if (PyRun_SimpleString(command_str.c_str()) == 0)
    ...
    YT_ABORT("Importing YT inline analysis script \"%s\" ... failed (please do not "

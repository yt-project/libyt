...
//
// Note        :  1. Overloaded with various data types: float, double, int, long,
// unsigned int, unsigned long
//                   ==> (float,double)                        are converted to double
//                   internally
//                       (int,long,unsigned int,unsigned long) are converted to long
//                       internally (long long)                           are converted to
...
int AddScalarToDict(PyObject* dict, const char* key, const T value) {
  ...
  if (typeid(T) == typeid(float) || typeid(T) == typeid(double)) {
    ...
    YT_ABORT("Unsupported data type (only support float, double, int, long, long long, "
             "unsigned int, unsigned long)!\n");
  }
  ...
  if (PyDict_SetItemString(dict, key, py_obj) != 0) {
    Py_DECREF(py_obj);
    YT_ABORT(
        "Inserting a dictionary item with value \"%.5g\" and key \"%s\" ... failed!\n",
        (double)value,
        key);
  }
...
// Description :  Auxiliary function for adding an n-element vector item to a Python
// dictionary
//
// Note        :  1. Overloaded with various data types: float, double, int, long,
// unsigned int, unsigned long
...
int AddStringToDict(PyObject* dict, const char* key, const char* string) {
  ...
  if (PyDict_SetItemString(dict, key, py_obj) != 0) {
    Py_DECREF(py_obj);
    YT_ABORT(
        "Inserting a dictionary item with string \"%s\" and key \"%s\" ... failed!\n",
        string,
        key);
  }
  Py_DECREF(py_obj);
...
template int AddScalarToDict<long long>(PyObject* dict, const char* key,
                                        const long long value);
template int AddScalarToDict<unsigned int>(PyObject* dict, const char* key,
                                           const unsigned int value);
template int AddScalarToDict<unsigned long>(PyObject* dict, const char* key,
                                            const unsigned long value);
template int AddVectorNToDict<float>(PyObject* dict, const char* key, const int len,
                                     const float* vector);
template int AddVectorNToDict<double>(PyObject* dict, const char* key, const int len,
                                      const double* vector);
template int AddVectorNToDict<int>(PyObject* dict, const char* key, const int len,
                                   const int* vector);
template int AddVectorNToDict<long>(PyObject* dict, const char* key, const int len,
                                    const long* vector);
template int AddVectorNToDict<long long>(PyObject* dict, const char* key, const int len,
                                         const long long* vector);
template int AddVectorNToDict<unsigned int>(PyObject* dict, const char* key,

...
//
// Notes       :  1. This struct will be use in yt_grid data member field_data and
// particle_data.
//
// Data Member :  data_ptr           : data pointer
//                data_dimensions[3] : dimension of the data to be passed to python for
//                wrapping, which is the actual
//                                     size of the array.
//                                     Def => fieldData[ dim[0] ][ dim[1] ][ dim[2] ]
//                data_dtype         : Data type of the field in specific grid. If this is
//                set as YT_DTYPE_UNKNOWN,
//                                     then we will use field_dtype define in field_list
...
//
// Notes       :  1. We assume that each element in array[3] are all in use, which is we
// only support
//                   dim 3 for now, though it can be [len][0][0].
//
// Data Member :  grid_dimensions : Number of cells along each direction in [x][y][z]
// coordinate.
//                left_edge       : Grid left  edge in code units
//                right_edge      : Grid right edge in code units
//                par_count_list  : Array that records number of particles in each
//                species, the input order
...
//                field_data      : Pointer pointing to yt_data array, each element stores
//                an info for
//                                  field array to be wrapped.
//                particle_data   : Pointer to pointer pointing to yt_data array
//                (yt_data** array),

...
// Description :  Data structure to store each species names and their number of
// attributes.
//
// Notes       :  1. Some data are overlap with yt_particle. We need this first be input
// by user through
//                   yt_set_Parameters(), so that we can set up and initialize
//                   particle_list properly.
//                2. For now, libyt only borrows the particle type par_type from
//                simulation. The lifetime
...
//
// Notes       :  1. The lifetime of attr_name should cover the whole in situ analysis
// process.
//                2. The lifetime of attr_unit, attr_name_alias, attr_display_name should
//                cover yt_commit
//                3. "attr_unit", "attr_name_alias", "attr_display_name", are set
//                corresponding to yt
//                   ( "name", ("units", ["alias1", "alias2"], "display_name"))
//
// Data Member :  const char   *attr_name           : Particle label name, which in yt, it
// is its attribute.
//                yt_dtype      attr_dtype          : Attribute's data type. Should be
//                yt_dtype. const char   *attr_unit           : Set attr_unit if needed,
//                if not set, it will search
//                                                    for XXXFieldInfo. Where XXX is set
//                                                    by
//                                                    LibytProcessControl::Get().param_yt_.frontend.
//                int           num_attr_name_alias : Set attribute name to alias, number
//                of the aliases. const char  **attr_name_alias     : Aliases. const char
//                *attr_display_name   : Set display name on the plottings, if not set, yt
//                will
...
typedef struct yt_attribute {
  ...
  //
  // Note        : 1. Initialize attr_unit as "". If it is not set by user, then yt will
  // use the particle
  //                  unit set by the frontend in yt_set_Parameters(). If there still
  //                  isn't one, then it will use "".

#ifndef __YT_TYPE_ARRAY_H__
#define __YT_TYPE_ARRAY_H__


/*******************************************************************************
/
/  yt_array structure
/
/  ==> included by yt_type.h, yt_type_field.h, yt_type_particle.h
/
********************************************************************************/

//-------------------------------------------------------------------------------------------------------
// Structure   :  yt_array
// Description :  Data structure to store derived fields and particle attributes generated by derived
//                functions or get particle attribute functions.
//
// Notes       :  1. This struct is used in yt_field data member derived_func and derived_func_with_name.
//                2. This struct is used in yt_particle data member get_attr.
//
// Data Member : gid                : gid of the field.
//               data_length        : size of the data_ptr.
//               data_ptr           : data pointer, where function should fill in the required data.
//
// Methods     : yt_array            : Constructor
//               ~yt_array           : Destructor, does nothing for now.
//-------------------------------------------------------------------------------------------------------
struct yt_array
{
    long      gid;
    long      data_length;
    void     *data_ptr;

    //===================================================================================
    // Method      :  yt_array
    // Description :  Constructor of the structure "yt_array"
    //
    // Note        :  Initialize all data members
    //
    // Parameter   :  None
    //===================================================================================
    yt_array()
    {
        gid = -1;
        data_length = 0;
        data_ptr = NULL;
    }
    ~yt_array()
    {

    }
};

#endif // #ifndef __YT_TYPE_ARRAY_H__

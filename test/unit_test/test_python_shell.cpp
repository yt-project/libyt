...
class PythonFixture : public testing::Test {
 private:
  ...
  void SetUp() override {
    ...
    std::cout << "LibytPythonShell will execute code in namespace '" << script_ << "'"
              << std::endl;
 ...
 protected:
  ...
  void InitializeAndImportScript(const std::string& script) const {
    if (mpi_rank_ == 0) {
      ...
      if (stat(script_fullname.c_str(), &buffer) == 0) {
        ...
        python_script.close();
        std::cout << "Empty test script '" << script_fullname << "' ... created"
                  << std::endl;
      }
    }
#ifndef SERIAL_MODE
  ...
  static PyObject* CreateTemplateDictStorage() {
    ...
    PyObject* py_sys = PyImport_ImportModule("sys");
    PyObject* py_template_dict_storage =
        PyObject_GetAttrString(py_sys, "TEMPLATE_DICT_STORAGE");
    Py_DECREF(py_sys);
...
class TestCheckCodeValidityAssumption :
    public PythonFixture,
    public testing::WithParamInterface<std::string> {};
...
TEST_F(TestPythonExecution, AllExecutePrompt_can_execute_rvalue_empty_code) {
  ...
  if (GetMpiRank() == src_mpi_rank) {
    status = python_shell_.AllExecutePrompt(
        "", "<test>", src_mpi_rank, output, output_mpi_rank);
  } else {
    status =
        python_shell_.AllExecutePrompt("", "", src_mpi_rank, output, output_mpi_rank);
  }
...
TEST_F(TestPythonExecution, AllExecutePrompt_can_execute_a_valid_single_statement) {
  ...
  if (GetMpiRank() == src_mpi_rank) {
    status = python_shell_.AllExecutePrompt(
        src_code, "<test>", src_mpi_rank, output, output_mpi_rank);
  } else {
    status =
        python_shell_.AllExecutePrompt("", "", src_mpi_rank, output, output_mpi_rank);
  }
...
TEST_F(TestPythonExecution, AllExecuteFile_can_execute_a_valid_arbitrary_code) {
  ...
  PythonStatus status;
  if (GetMpiRank() == src_mpi_rank) {
    status = python_shell_.AllExecuteFile(
        src_code, "<test>", src_mpi_rank, output, output_mpi_rank);
  } else {
...
TEST_F(TestPythonExecution, AllExecuteCell_can_execute_empty_code) {
  ...
  PythonStatus status;
  if (GetMpiRank() == src_mpi_rank) {
    status = python_shell_.AllExecuteCell(
        src_code, "<test>", src_mpi_rank, output, output_mpi_rank);
  } else {
...
TEST_P(TestCheckCodeValidity, Can_distinguish_user_not_done_yet_error) {
  ...
  CodeValidity code_validity =
      LibytPythonShell::CheckCodeValidity(not_done_code, true, "<test>");
...
INSTANTIATE_TEST_SUITE_P(
    IsIncompleteInPrompt, TestCheckCodeValidity,
    testing::Values(
        std::string("if 1==1:\n"), std::string("if 1==1:\n  pass\nelse:\n"),
        std::string("if 1==1:\n  pass\nelif 2==2:\n"), std::string("try:\n"),
        std::string("try:\n  pass\nexcept:\n"), std::string("try:\n  pass\nfinally:\n"),
...
INSTANTIATE_TEST_SUITE_P(
    ErrorMsgHasNumberAtTheEnd, TestCheckCodeValidityAssumption,
    testing::Values(
        std::string("if 1==1:\n"), std::string("if 1==1:\n  pass\nelse:\n"),
        std::string("if 1==1:\n  pass\nelif 2==2:\n"), std::string("try:\n"),
        std::string("try:\n  pass\nexcept:\n"), std::string("try:\n  pass\nfinally:\n"),

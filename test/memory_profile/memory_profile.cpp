...
#define CALL_VALGRIND(name_tag, my_rank, time_tag)                                       \
  {                                                                                      \
    ...
    snprintf(valgrind,                                                                   \
             100,                                                                        \
             "detailed_snapshot %s_rank%d_time%d.mem_prof\0",                            \
             name_tag,                                                                   \
             my_rank,                                                                    \
             t);                                                                         \
...
void DerivedFunc(const int len, const long* list_gid, const char* field_name,
                 yt_array* data_array) {
  for (int l = 0; l < len; l++) {
...
// void GetParticleAttr(const int list_len, const long *gid_list, const char *par_type,
...
int main(int argc, char* argv[]) {
  ...
  for (int t = 0; t < iter; t++) {
    ...
    for (long lid = 0; lid < num_grids_local; lid++) {
      ...
      grid_idx[1] = (gid - (grid_idx[2] * param_yt.domain_dimensions[0] *
                            param_yt.domain_dimensions[1])) /
                    param_yt.domain_dimensions[0];
      grid_idx[0] =
          gid -
          (grid_idx[2] * param_yt.domain_dimensions[0] * param_yt.domain_dimensions[1]) -
      ...
      for (int d = 0; d < 3; d++) {
        grids_local[lid].field_data[0].data_dimensions[d] =
            grids_local[lid].grid_dimensions[d];
      }
